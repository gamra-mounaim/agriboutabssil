...
export default function SupplierList({ permissions }: { permissions: any }) {
  ...
  const addSupplier = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ text: language === 'ar' ? "فشل إضافة المورد." : language === 'fr' ? "Échec de l'ajout." : "Failed to add supplier.", type: 'error' });
      console.error(err);
    }
  };
  ...
  const handleAdjustSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (adjustModal.type === 'pay') {
        ...
        setMessage({ text: language === 'ar' ? "تم تسجيل الدفع للمورد." : language === 'fr' ? "Paiement enregistré." : "Payment to supplier posted successfully.", type: 'success' });
      } else {
        await api.addSupplierCharge(adjustModal.supplier.id, amt, adjustNote);
        setMessage({ text: language === 'ar' ? "تمت إضافة الدين." : language === 'fr' ? "Dette ajoutée." : "Supplier debt updated successfully.", type: 'success' });
      }
      ...
      setMessage({ text: language === 'ar' ? "فشلت العملية." : language === 'fr' ? "Opération échouée." : "Operation failed.", type: 'error' });
      console.error(err);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 px-4 pb-20">
      ...
      {/* ✅ FIXED TABS — pill-style, works in light & dark mode */}
      <div className="flex gap-2 p-1.5 bg-bg-base/70 border border-border-subtle rounded-2xl w-fit backdrop-blur-sm">
        <button
          onClick={() => setActiveTab('suppliers')}
          className={cn(
            "px-5 py-2 rounded-xl font-black text-[11px] tracking-widest transition-all duration-200 uppercase",
            activeTab === 'suppliers'
              ? "bg-slate-800 dark:bg-slate-100 text-white dark:text-slate-900 shadow-md"
              : "text-text-secondary hover:text-text-main hover:bg-white/70 dark:hover:bg-white/5"
          )}
        >
          {t.suppliers}
        </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredSuppliers.map(s => (
              <div key={s.id} className="bg-card border border-border-subtle p-6 rounded-2xl shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
                ...
                <div className={cn("p-4 rounded-xl mb-6", s.debt > 0 ? "bg-red-50 dark:bg-red-900/10" : "bg-emerald-50 dark:bg-emerald-900/10")}>
                ...
                {canViewDebtAmount && (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button onClick={() => setAdjustModal({ type: 'charge', supplier: s })} className="bg-white dark:bg-white/5 border border-border-subtle text-text-main py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:border-accent transition-all">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="bg-card w-full max-w-md rounded-2xl shadow-2xl p-8 border border-border-subtle">
              ...
              <form onSubmit={handleAdjustSubmit} className="space-y-6">
                ...
                <div className="flex gap-4 pt-4">
                  <button type="button" onClick={() => setAdjustModal(null)} className="flex-1 bg-white dark:bg-white/5 border border-border-subtle text-text-secondary py-4 rounded-2xl font-black text-xs tracking-widest active:scale-95 transition-all">{t.cancel}</button>
