    ...
    <div className="max-w-6xl mx-auto space-y-8 px-4 pb-12">
      <div className={cn("flex flex-col md:flex-row md:items-end justify-between gap-4", language === 'ar' && "md:flex-row-reverse")}>
        <div className={cn(language === 'ar' && "text-right")}>
          ...
          <p className="text-text-secondary text-xs font-mono uppercase tracking-widest">{t.checkManagement}</p>
        </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-bg-base/50 border-b border-border-subtle text-center">
              <th className="p-5 text-[10px] font-black uppercase text-text-secondary tracking-widest">{t.checkNumber}</th>
              <th className="p-5 text-[10px] font-black uppercase text-text-secondary tracking-widest">{t.checkOwner}</th>
              <th className="p-5 text-[10px] font-black uppercase text-text-secondary tracking-widest">{t.checkPartyCustomer} / {t.checkPartySupplier}</th>
              <th className="p-5 text-[10px] font-black uppercase text-text-secondary tracking-widest">{t.checkCategory}</th>
              <th className="p-5 text-[10px] font-black uppercase text-text-secondary tracking-widest">{t.amount}</th>
                  ...
                  <td className="p-5">
                    <div className="flex flex-col items-center">
                      ...
                      <span className={cn("text-[8px] font-black px-2 py-0.5 rounded uppercase mt-1", 
                        ...
                        {check.partyRole === 'customer' ? t.checkPartyCustomer : t.checkPartySupplier}
                      </span>
                    </div>
                  </td>
                  <td className="p-5">
                    <div className={cn("text-[9px] font-black uppercase px-2 py-0.5 rounded-md inline-block", 
                      ...
                      {check.type === 'sale' ? t.saleCheck : 
                       check.type === 'payment' ? t.paymentCheck :
                       t.supplierPayCheck}
                    </div>
                  </td>
                  <td className="p-5">
                    <div className={cn("inline-block px-3 py-1 rounded-full text-xs font-black", 
                      ...
                      {check.partyRole === 'customer' ? '+' : '-'}{formatNumber(check.total)} {t.currency}
                    </div>
                  </td>
                  <td className="p-5 text-right font-mono text-[11px] text-text-secondary">
                    {new Date(check.date).toLocaleString(language === 'ar' ? 'ar-EG' : language === 'fr' ? 'fr-FR' : 'en-US')}
                  </td>
                </tr>
              ))
            )}
